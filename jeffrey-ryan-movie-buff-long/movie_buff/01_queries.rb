def it_was_ok
  ...
  # Movie.select('id, title, score').where('score >= ? AND score <= ?', 2, 3)
...
def harrison_ford
  ...
  # as a lead actor.
  Movie.select('movies.id, movies.title').distinct.joins(:actors,:castings).where('actors.name = (?)', 'Harrison Ford').where.not('castings.ord = (?)', 1)
end
...
def biggest_cast
  ...
  # actors).
  Movie.select('movies.id, movies.title').group('movies.id').joins(:castings).order('COUNT(castings.actor_id) DESC').limit(3)
